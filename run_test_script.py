...
# print(glensCntrlPoi['lev'][len(glensCntrlPoi['lev'].data)-1-16])
# print(len(glensCntrlPoi['lev'].data)-1-16)
# sys.exit('Leveled')
...
levOfInt = glensCntrlPoi['lev'].data[len(glensCntrlPoi['lev'].data)-1-16]
levs = glensCntrlPoi['lev'].data
levMask = levs > levOfInt
print(levOfInt)
print(levMask)
# print(glensFdbckPoi['lon'])
# sys.exit('longituded')
...
if regionToPlot == 'point':
    ...
    fdbckToPlot = glensFdbckPoi[:,levMask,:,:]
    # print(fdbckToPlot['lev'].data)
    fdbckToPlot = glensFdbckPoi.sel(lat=latOfInt, lon=lonOfInt, method="nearest")
    fdbckToPlot = fdbckToPlot.sum(dim='lev')
    print(fdbckToPlot.data)
    # fdbckToPlot = glensFdbckPoi.sel(lat=latOfInt, lon=lonOfInt, method="nearest")
...
plt.legend()
plt.ylabel(yStr)
plt.title('TropCO' + ': ' + startStr + '-' + endStr + ' ' + locTitleStr)
...
plt.savefig(saveName + locStr + '_' + '.png',dpi=dpi_val,bbox_inches='tight')
